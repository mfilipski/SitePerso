$TITLE LESOTHO LEWIE MODEL
* Mateusz Filipski, June 2012

* This model makes tables 9.8 , 9.9 and 9.10 in Beyond Experiments.
...
* = Parameters with a "_dr" suffix ("draw") are the inputs to each round of simulation. They were either
...
* = Parameters with a "1" suffix are the values generated from the calibration run of each drawn model. In theory
...
* = Parameters with a "PC" suffix are the % changes from "1" level.  ex: tqpPC(g,draw) = 100*tqpD(g,draw)/tqp1(g,draw)
...
* = Parameters with a "_mv" suffix ("mean & varience") contain MEANS, STDEV, and for some PERCENTILES of the parameters that are indexed by draw.
...
$call "gdxxrw input=Lesotho_LEWIE_input5.xlsx output=LesothoLEWIEData5.gdx index=Index!A2"
...
* "ac" are all the accounts of the SAM, all account names must appear in that column (important if once wants to change the inputs).
* similarly, column "Input!C3" is where GAMS reads the names of all the goods, all commodity account names must appear there
* etc. for all these elements.

* the $gdxin opens the data loading procedure and calls the .gdx file we just made
$gdxin LesothoLEWIEData5.gdx
...
* (in most cases, there is no need to change anything else in the model but these following 6 items )
...
* 3) Choose whether or not to have a budget constraint on agricultural input purchases using the $setglobal budgetconstraint
...
* 6) Choose which households receive their transfer using the ht subset.
* The PILOT model gives a transfer to A only.
* The SCALE-UP would give it to both A and B.  C and D wouldn't receive a transfer, they are uneligible.
* -------------------------------------------------------------------------------------------------

* ##############################################################################################
* ################################ TABLES IN THE CHAPTER #######################################
* ##############################################################################################
* TABLE 9.8 (base simulation) gets generated as a text file "table9pt8.txt"
* (setlocals hlse 100 flse 100 budgetconstraint 1, loosencapital 0
* the exact values are obtained with 499 draws, but 12 draws give results similar enough)

* Values for Table 9.9 is read directly in the .lst file, prodmult_treat_mv and prodmult_ntreat_mv

* To generate column 1 of 9.10 (constrained simulation) set values as follows:
*hlse 100, flse 100, budgetconstraint  loosencapital 1
* To generate column 2 of 9.10 (unconstrained simulation) set values as follows:
*hlse 0, flse 0, budgetconstraint 1 loosencapital 0
* ##############################################################################################
* ##############################################################################################
...
* Choose whether or not to loosen the capital
* 0 = fixed capital, 1 = loosen up capital
$setglobal loosencapital 0
* ##############################################################################################
* ##############################################################################################
...
* goods subsets
     ...
     gp(g)     goods that are produced / crop, ret, ser, live, prod /
     gag(g)    ag goods /crop, live/
     gnag(g)   non ag goods /ret, ser, prod /

* household subsets
     ht(h)     recipients in this simulation (can be a or b or both) / a, null/
...
sets
     ...
     maphv mapping housheold to their village / (a,c).T
                                                 (b,d).NT /
;
...
* Parameters starting with "xl" are smaller, more tractable parts of the big input spreadsheet (alldata)
...
parameter
* meta-parameters with parameter draws (all named with "_dr" suffix)
...
* calibration values in each draw (with "1" suffix)
...
tqp1(g,draw)        calibrated total quantity produced by good
ttqp1(draw)         calibrated total value of production in the economy
...
* after a simulation for each draw ("2" suffix)
...
ttqp2(draw)         simulated total value of production in the economy
hqp2(h,draw)        simulated total qty produced by a household
...
* delta calibration /simulation ("D" suffix)
...
ttqpD(draw)         delta total value of production in the economy
...
* percent change calibration/simulation  ("PC" suffix)
...
ttqpPC(draw)         % change in total value of production in the economy
...
* just for info, display the negatives in cobb-douglas (should be rare):
...
display shcobb_t;
* once no negative values drawn, we can use that as our parameter draw:
...
* ------------------------------------------------------------
* just for info, display the negatives (should be rare)
...
*---------------------------------------------------------------------------------------------------------
* two conditions, hence the loop in the loop:
...
* ALL OTHER PARAMETERS EITHER FOLLOW FROM THOSE DRAWS
* EITHER BY DEFINITION OR AS A RESULT OF ASSUMPTIONS/CLOSURE RULES
...
* START FROM INCOME (in spreadsheet):
option decimals = 2 ;
...
ABORT$(card(bigtr_dr)) "These households spend over 10% of income on transfers", bigtr_dr ;
set smallzoi_dr(h,draw);
smallzoi_dr(h,draw)$(expzoish_dr(h,draw) < 0.5) = yes ;
ABORT$(card(smallzoi_dr)) "These households spend less than half of their income in the zoi", smallzoi_dr ;

* LEVELS OF CONSUMPTION:
* disposable income spent according to budget shares
...
* intermediate demand requirements (share by good, then total share)
idsh_dr(gg,g,h,draw) = xlVA2IDsh(gg,g,h)/(1+xlVA2IDsh(gg,g,h));
...
* But QP and ID must be jointly determined (since ID increases TQP which increases ID which increases TQP etc.)
* NB: We initialise QP and ID at plausible values, but then we use an NLP solve to refine them.
parameter netexpsh(g) net export share of a good out of the zoi;
* (averaged accross households)
netexpsh(g)$gnag(g) = 1-(1/card(h)*(sum(h,xlrevsh_vil(g,h)+xlrevsh_zoi(g,h)))) ;
display netexpsh ;

* temporary parameter to store the values of the NLP
parameter tempid_dr(g,draw) temporary total intermediate demand;
tempid_dr(g,draw) = sum((gg,h),qc_dr(gg,h,draw)*(idsh_dr(g,gg,h,draw)/(1-idsh_dr(g,gg,h,draw)))) ;
display tempid_dr ;

* now determine total QP
tqp_dr(g,draw) = [sum(h, qc_dr(g,h,draw)) + tempid_dr(g,draw) ]/(1-netexpsh(g)) ;
ttqp_dr(draw)= sum(g,tqp_dr(g,draw));
display tqp_dr, ttqp_dr ;

* split qp in each household according to their capital shares or consumption share:
parameter qpshare(h,g) share of household h in production of g ;
qpshare(h,g)$gnag(g) = xlFD(g,"K",h) / sum(hh,xlFD(g,"K",hh)) ;
qpshare(h,g)$gag(g) = qc_dr(g,h,"dr0")/sum(hh,qc_dr(g,hh,"dr0")) ;
display qpshare ;
qp_dr(g,h,draw) = tqp_dr(g,draw) * qpshare(h,g) ;
display qp_dr ;

* That determines intermediate demands:
id_dr(gg,g,h,draw) = qp_dr(g,h,draw) * idsh_dr(gg,g,h,draw) ;
display id_dr ;


* USE A MINI SOLVE STATEMENT TO FIGURE OUT THE PRODUCTION SIDE QP/ID balance:
*---------------------------------------------------------------------------
variables
         NETEXPTEMP(g,draw)
         QPTEMP(g,h,draw)
...
equations
     NETEXPTEMP_eq(g,draw)  net expenditures
     TQPTEMP_eq(g,draw)     total quantities produced
     QPTEMP_eq(g,h,draw)    quantities produced
     IDTEMP_eq(g,gg,h,draw) intermediate demand
     MKTCLR_mini(g,draw)    market clearing
     FAKEQ                  fake equation to pretend we are maximizing;
...
* The model solves for a balanced system of production and intermediate demands:
* These are our updated values:
...
* Value added share for all activities
...
* minimum consumption: zero for lack of data.
cmin_dr(g,h,draw) = 0 ;
...
* last missing: exinc_dr THE PARAMETER THAT CLEARS THE MATRIX
...
* exogenous income is the difference between total income and the value of endowments:
exinc_dr(h,draw) =  y_dr(h,draw) - feinc_dr(h,draw) ;
...
* THE SAM IS ONLY A BY-PRODUCT OF THE MODEL (RATHER THAN THE INPUT TO IT)
...
* This unloads the parameter into a .gdx data file: (unstar if needed)
execute_unload "outmat.gdx" outmat ;
* And this writes in an excel sheet called "MakeMeASam": (unstar if needed)
execute "gdxxrw.exe outmat.gdx par=outmat o=MakeMeASam.xlsx rng=a1:am39 rdim=3 cdim=3"
* (NB: the range rng=a1:am39 is the exact size of the Lesotho matrix)
...
* The zero draw dr0 is using the mean values, all subsequent drXX are randomely drawn from distributions.
loop(draw,
* re-initialise all the variables in the matrix at the _dr levels
...
* RE-CALIBRATION
*---------------------------------
* set iterlim to 1 - this is to check that the data inputs form an economy already at equilibrium
* i.e. they are a solution to the model without any need to GAMS to change any values
option iterlim = 1 ;
...
* CASH TRANSFER:
transfer(h) = 0 ;
* from Taylor_Thome_rev.docx  p.13  amount of transfer is 3,310,560 (to household A).
* (When scaled up, household B will be getting 3,245,760)
* This gives households in the "ht" set their transfer.
transfer("a")$ht("a") = 3310560 ;
exinc("a") = exinc("a") + transfer("a")  ;
* Note: Unless you put "b" into the ht set (controlled at the top), its transfer remains zero
transfer("b")$ht("b") = 3245760 ;
exinc("b") = exinc("b") + transfer("b") ;


* LOOSEN CAPITAL:
* This loosens capital by exactly the value that keeps wage inflation at zero (or negligible)
* NB: These values depend on other assumptions, are scenario-specific, and should be found by tatonnement.
if(%loosencapital% = 1,
     fixfac(g,"K",h)$(maphv(h,"T")) = fixfac(g,"k",h)*1.036 ;
     fixfac(g,"land",h)$(maphv(h,"T")) = fixfac(g,"land",h)*1.043 ;
);
...
* ================================================================================================
* ================================================================================================
* Compute and display all the values, differences in values, standard errors, etc...
...
ymult(draw) = sum(h,yD(h,draw)) / sum(h,transfer(h)) ;
cvmult(draw) = cv(draw) / sum(h,transfer(h)) ;

ymult_h(h,draw)$transfer(h) = yD(h,draw) / transfer(h) ;
ytotmult_h(h,draw) = yD(h,draw) /sum(hh, transfer(hh)) ;
ymult_treat(draw)  = sum(h$transfer(h),yD(h,draw)) / sum(h, transfer(h)) ;
ymult_vil(v,draw)$sum(hh$maphv(hh,v),transfer(hh))  = sum(h$maphv(h,v),yD(h,draw)) / sum(hh$maphv(hh,v),transfer(hh))  ;
ymult_all(draw)$sum(h,transfer(h))  = sum(h,yD(h,draw)) / sum(h,transfer(h)) ;

rymult_h(h,draw)$transfer(h) = ryD(h,draw) / transfer(h) ;
rytotmult_h(h,draw) = ryD(h,draw) /sum(hh, transfer(hh)) ;
rymult_treat(draw)  = sum(h$transfer(h),ryD(h,draw)) / sum(h, transfer(h)) ;
rymult_vil(v,draw)$sum(hh$maphv(hh,v),transfer(hh))  = sum(h$maphv(h,v),ryD(h,draw)) / sum(hh$maphv(hh,v),transfer(hh))  ;
rymult_all(draw)$sum(h,transfer(h))  = sum(h,ryD(h,draw)) / sum(h,transfer(h)) ;

prodmult_h(g,h,draw)$transfer(h) = qpD(g,h,draw) / transfer(h) ;
prodTotmult_h(g,h,draw) = qpD(g,h,draw) / sum(hh,transfer(hh)) ;
prodmult_treat(g,draw)  = sum(h$transfer(h),qpD(g,h,draw)) / sum(h, transfer(h)) ;
prodmult_ntreat(g,draw)  = sum(h$(not transfer(h)),qpD(g,h,draw)) / sum(h, transfer(h)) ;
prodmult_vil(g,v,draw)$sum(hh$maphv(hh,v),transfer(hh))  = sum(h$maphv(h,v),qpD(g,h,draw)) / sum(hh$maphv(hh,v),transfer(hh))  ;
...
prodVmult_h(g,h,draw)$transfer(h) = (ph2(g,h,draw)*qpD(g,h,draw)) / transfer(h) ;
prodVmult_treat(g,draw)  = sum(h$transfer(h),(ph2(g,h,draw)*qpD(g,h,draw))) / sum(h, transfer(h)) ;
prodVmult_ntreat(g,draw)  = sum(h$(not transfer(h)),(ph2(g,h,draw)*qpD(g,h,draw))) / sum(h, transfer(h)) ;
prodVmult_vil(g,v,draw)$sum(hh$maphv(hh,v),transfer(hh))  = sum(h$maphv(h,v),(ph2(g,h,draw)*qpD(g,h,draw))) / sum(hh$maphv(hh,v),transfer(hh))  ;
prodVmult_all(g,draw)$sum(h,transfer(h))  = sum(h,(ph2(g,h,draw)*qpD(g,h,draw))) / sum(h,transfer(h)) ;
...
ykmult_all(draw)$(sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) = tyD(draw)
 / (sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) ;
rykmult_all(draw)$(sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) = tryD(draw)
 / (sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) ;
display ymult_all, rymult_all, ykmult_all, rykmult_all ;

ykmult_h(h,draw)$(transfer(h)+sum((g,f),fixfacD(g,f,h,draw))) = yD(h,draw)
 / (transfer(h)+sum((g,f),fixfacD(g,f,h,draw))) ;
rykmult_h(h,draw)$(transfer(h)+sum((g,f),fixfacD(g,f,h,draw))) = ryD(h,draw)
 / (transfer(h)+sum((g,f),fixfacD(g,f,h,draw))) ;
ykTotmult_h(hh,draw)$(sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) = yD(hh,draw)
 / (sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) ;
rykTotmult_h(hh,draw)$(sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) = ryD(hh,draw)
 /(sum(h,transfer(h))+sum((h,g,f),fixfacD(g,f,h,draw))) ;
...
ttqpD_mv(mv)         mean and stdev of delta total-total QP
...
hqpD_mv(h,mv)        mena and stdev of delta hqp
;
...
hqpPC_mv(h,mv)        mean and stdev of % change in hqp
;
...
display ymult_all_mv, rymult_all_mv, ytotmult_h_mv, rytotmult_h_mv, prodmult_all_mv , ttprodmult_mv,
       hprodmult_mv, prodmult_treat_mv,  prodmult_ntreat_mv;
...
* Real Multipliers at the household level
loop(h,
...
* Production multipliers, overall
Torank(draw) = ttprodmult(draw) ;
ci2(lh) = ci(lh);
$libinclude rank Torank draw Ranks ci2
ttprodmult_mv(lh) = ci2(lh) ;

* Production multipliers, by household
loop(h,
...
* Production multipliers, by good
loop(g,
...
* These two parameters are used to make Table 9.9 (production multipliers for the treated and non-treated)
...
* It makes a text file (table9pt8.txt) which can be easily cut and pasted into excel.
...
file tablput20_4bk /table9pt8.txt/;
put tablput20_4bk ;
...
put 'Transfer' /;
put 'A'  @40'; '  exincD_mv("a","mean"):<12:0 /;
put 'B'  @40'; '  exincD_mv("b","mean"):<12:0 /;
put //;
put 'iterations'  @40'; ' card(draw) /;
put /;


set hac(h) just a and c households /a,c/
...
loop(h$hac(h),
     ...
     put @40';' '(' cpiPC_mv(h,"pct5"):5:2 '%,' cpiPC_mv(h,"pct95"):5:2 '%)' /;
